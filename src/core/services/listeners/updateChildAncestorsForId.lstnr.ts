...
class UpdateChildAncestorsForIdListener {
  ...
  private TOPIC = '*.products.update.sideEffect';
  ...
  public start() {
    this.server.log.info(`${yellow('UpdateChildAncestorsForIdService')} ${green('starting in')} [${this.TOPIC}]`);
    this.server.messages.subscribe(this.TOPIC, this.handler.bind(this));
  }

  private handler = async (data: any) => {
    const repo = this.server.db.repo.classificationCategoryRepository;
